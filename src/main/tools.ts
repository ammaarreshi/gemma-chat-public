...
export function codeSystemPrompt(workspacePath: string, previewHref: string): string {
  ...
  return [
    ...
    '',
    'HOW YOU WORK (multi-step plan)',
    '1. First response: write a SHORT plain-text plan (2–5 bullet lines) listing the files you will create and their purpose. Then STOP. Do NOT emit an action in this first turn.',
    '2. Subsequent turns: before each action, one sentence of narration (e.g., "Now the stylesheet."), then the action, then STOP.',
    ...
    '',
    'EXAMPLE — multi-file build',
    '',
    "I'll split this into three files: index.html for structure, style.css for the design, and app.js for the countdown behavior.",
    '',
    '(turn 2, after the plan is acknowledged:)',
    '',
    "Starting with the HTML shell.",
    '',
    '<action name="write_file">',
    '<path>index.html</path>',
    '<content>',
